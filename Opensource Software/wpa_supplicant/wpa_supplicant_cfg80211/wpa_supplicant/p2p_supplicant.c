					...
					struct wpa_ssid *ssid)
{
	...
	for (iface = wpa_s->global->ifaces; iface; iface = iface->next) {
		...
		if (!wpas_is_p2p_prioritized(iface)) {
			 ...
			 */
/*
	Hisense MT5655+MT7603U project
	[DTV00621384][WCNAE00087127] Owner：MCD_WCN_WCD_SS5 Zheng Zhou (周正)
	[miracast]  wifi打开，扫描下wifi，但是不连接到AP，然后进行miracast连接，
				一般第一次连接OK，第二次进行miracast连接的时候，手机搜不到设备
	实际上：P2P连接成功后，wlan0端还有去尝试去连接AP，导致issue产生
	
	原因：wpa_supplicant在处理频率冲突时使用的iface接口并非wlan0，而是p2p1接口
	solution：修改成wlan0接口
*/
#if 1
			struct wpa_supplicant* iface_tmp = wpa_s->global->ifaces;
			while(iface_tmp->next)
			{
				iface_tmp = iface_tmp->next;
			}
			if(iface_tmp->current_ssid == NULL)  //sometimes, iface_tmp->current_ssid is NULL
			{	
				iface_tmp->current_ssid = ssid;
			}
			wpa_supplicant_disable_network(iface_tmp,
						       ssid);
			wpa_msg(iface_tmp, MSG_INFO,
				WPA_EVENT_FREQ_CONFLICT " id=%d", ssid->id);
			os_memset(iface_tmp->pending_bssid, 0,
				  ETH_ALEN);
#else
