  ...
  for (ResourceMapIndex::iterator mapiter = m_resourceMaps.begin();
       ...
       ++mapiter) {
        ...
        for (ResourceMap::iterator i = resourcemap->begin(); i != resourcemap->end(); ++i) {
            ...
            if (resource->response().isTurboCompressedImage(&used_image_quality) && used_image_quality < image_quality)
                to_evict.append(resource);
        }

        for (Vector<Resource*>::iterator i = to_evict.begin(); i != to_evict.end(); ++i) {
            MemoryCacheEntry* entry = resourcemap->get((*i)->url());
            evict(entry);
        }
    }
}
