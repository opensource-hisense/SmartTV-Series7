...
void setCallStack(blink::TracedValue* value)
{
    ...
    if (!*traceCategoryEnabled)
        return;
    RefPtrWillBeRawPtr<blink::ScriptCallStack> scriptCallStack = blink::currentScriptCallStack(blink::ScriptCallStack::maxCallStackSizeToCapture);
    if (scriptCallStack)
