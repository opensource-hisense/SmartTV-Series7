...
void HTMLPlugInElement::attach(const AttachContext& context)
{
    ...
    if (PluginView* plugin = getPluginView())
        plugin->didAttachToDocument(context.performingReattach);
...
void HTMLPlugInElement::createPluginWithoutLayoutObject()
{
    ...
    loadPlugin(document().completeURL(url), serviceType, paramNames, paramValues, useFallback, false);
}
...
void HTMLPlugInElement::detach(const AttachContext& context)
{
    ...
    if (PluginView* plugin = getPluginView())
        plugin->didDetachFromDocument(context.performingReattach);
