...
class DataCue final : public TextTrackCue, public ContextLifecycleObserver {
    ...
    WILL_BE_USING_GARBAGE_COLLECTED_MIXIN(DataCue);
public:
    static DataCue* create(ExecutionContext* context, double start, double end, PassRefPtr<DOMArrayBuffer> data)
    {
    ...
    PassRefPtr<DOMArrayBuffer> data() const;
    void setData(PassRefPtr<DOMArrayBuffer>);
...
protected:
    DataCue(ExecutionContext* , double start, double end, PassRefPtr<DOMArrayBuffer> data);
