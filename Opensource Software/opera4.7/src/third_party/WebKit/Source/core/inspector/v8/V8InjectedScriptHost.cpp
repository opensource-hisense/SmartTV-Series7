...
namespace {
bool functionDetailsForInvokableObject(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    if (info.Length() < 1 || info[0]->IsFunction() || !info[0]->IsObject())
        return false;

    v8::Local<v8::Object> functionObject = v8::Local<v8::Object>::Cast(info[0]);
    ...
    location->Set(v8AtomicString(isolate, "lineNumber"), v8::Integer::New(isolate, 0));
    location->Set(v8AtomicString(isolate, "columnNumber"), v8::Integer::New(isolate, 0));
    location->Set(v8AtomicString(isolate, "scriptId"), v8::Integer::New(isolate, 0)->ToString(isolate));

    v8::Local<v8::Object> result = v8::Object::New(isolate);
    v8::Local<v8::String> name = functionObject->ToString();
    ...
    result->Set(v8AtomicString(isolate, "isGenerator"), v8::Boolean::New(isolate, false));
    v8SetReturnValue(info, result);
    return true;
}
}
...
{
    if (functionDetailsForInvokableObject(info))
        return;
