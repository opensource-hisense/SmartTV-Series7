...
public:
        ...
        , m_textWrapEnabled(block.document().textWrapEnabled())
    {
        if (m_textWrapEnabled) {
            ...
            for (InlineWalker walker(toLayoutBlockFlow(m_block)); !walker.atEnd(); walker.advance()) {
...
inline bool BreakingContext::handleText(WordMeasurements& wordMeasurements, bool& hyphenated)
{
    ...
    LayoutObject* container = static_cast<LayoutObject*>(m_current.object())->container();
    ...
    bool breakWords = (m_currentStyle->breakWords() || (limitLineWidthForTextWrap && !m_currentStyle->textOverflow())) && ((m_autoWrap && !m_width.committedWidth()) || m_currWS == PRE);
    ...
    for (; m_current.offset() < layoutText.textLength(); m_current.fastIncrementInTextNode()) {
        ...
        if (betweenWords || midWordBreak) {
            ...
            if (!m_width.committedWidth() && m_autoWrap && !m_width.fitsOnLine() && !widthMeasurementAtLastBreakOpportunity)
                m_width.fitBelowFloats(m_lineInfo.isFirstLine(), limitLineWidthForTextWrap);
