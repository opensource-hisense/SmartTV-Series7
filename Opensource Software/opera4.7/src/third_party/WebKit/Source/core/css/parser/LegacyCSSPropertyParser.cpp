...
bool CSSPropertyParser::parseValue(CSSPropertyID unresolvedProperty, bool important)
{
    ...
    case CSSPropertyNavUp:
        if (id == CSSValueAuto) {
            ...
            if (value->m_unit != CSSParserValue::IdSel && value->m_unit != CSSParserValue::HexColor)
                return false;

            list->append(CSSIdSelValue::create(value->string));
            value = m_valueList->next();
            if (value && value->m_unit == CSSParserValue::String) {
                if (value->string.startsWithIgnoringCase("_"))
                    return false;
                list->append(CSSStringValue::create(value->string));
            }
        ...
        break;
    case CSSPropertyInputFormat: // auto | <string>
        ASSERT(RuntimeEnabledFeatures::cssInputFormatEnabled());
        if (id == CSSValueAuto) {
            validPrimitive = true;
        } else if (value && value->m_unit == CSSParserValue::String) {
            addProperty(propId, CSSStringValue::create(value->string), important);
            return true;
        }
        break;
