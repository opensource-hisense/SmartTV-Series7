    ...
    while (!range.atEnd()) {
        ...
        case UrlToken: {
           ...
           if (token.type() == HashToken) {
                bool hashTokenIsColor = true;
                for (size_t i = 0; i < token.value().length(); ++i) {
                    if (!isASCIIHexDigit(token.value()[i])) {
                        hashTokenIsColor = false;
                        break;
                    }
                }
                value.m_unit = hashTokenIsColor ? static_cast<int>(CSSParserValue::HexColor) : static_cast<int>(CSSParserValue::IdSel);
