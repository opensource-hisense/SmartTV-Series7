...
void XMLHttpRequest::createRequest(PassRefPtr<EncodedFormData> httpBody, ExceptionState& exceptionState)
{
    ...
    if (executionContext()->isDocument() && document()->settings() && !document()->settings()->XHROriginCheckEnabled()) {
        originCheckEnabled = false;
    }
    m_sameOriginRequest = !originCheckEnabled || securityOrigin()->canRequestNoSuborigin(m_url);
