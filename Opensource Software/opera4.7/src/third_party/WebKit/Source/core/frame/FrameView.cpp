...
void FrameView::forcePluginUpdates()
{
    Vector<RefPtr<LayoutPart> > parts;
    copyToVector(m_parts, parts);

    for (size_t i = 0; i < parts.size() && layoutView(); ++i) {
        if (parts[i]->widget() && parts[i]->widget()->isPluginContainer()) {
            IntRect rect = parts[i]->widget()->frameRect();
            IntRect offsetRect(rect);
            offsetRect.setX(rect.x() + 1);
            // Offset the plugin by 1 pixel so that it has to report movement.
            parts[i]->widget()->setFrameRect(offsetRect);
            // Move the widget back to its original position.
            parts[i]->widget()->setFrameRect(rect);
        }
    }
}
...
void FrameView::setTextWrapWidth(int textWrapWidth)
{
    ...
    for (LayoutObject* renderer = layoutView(); renderer; renderer = renderer->nextInPreOrder()) {
