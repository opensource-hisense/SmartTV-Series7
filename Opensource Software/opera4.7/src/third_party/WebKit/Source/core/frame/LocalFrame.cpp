...
inline float parentPageZoomFactor(LocalFrame* frame, float defaultScaleFactor)
{
    ...
    const GlobalPaintFlags globalPaintFlags, IntRect paintingRect, float opacity)
{
    ...
    return DragImage::create(image.get(), shouldRespectImageOrientation, deviceScaleFactor * m_host->altDeviceScaleFactor());
}
...
inline LocalFrame::LocalFrame(FrameLoaderClient* client, FrameHost* host, FrameOwner* owner)
    ...
    , m_pageZoomFactor(parentPageZoomFactor(this, host->altDeviceScaleFactor()))
