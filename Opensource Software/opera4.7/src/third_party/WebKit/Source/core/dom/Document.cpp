...
ContentEditablesController* Document::contentEditablesController()
{
    if (!m_contentEditablesController) {
        ...
        if (m_frame && m_frame->loader().currentItem() && m_frame->loader().currentItem()->isCurrentDocument(this))
            m_frame->loader().currentItem()->setContentEditablesState(contentEditableElementsState());
    }
...
void Document::setStateForNewContentEditableElements(const Vector<String>& stateVector)
{
    if (!stateVector.size())
        return;
...
bool Document::textWrapEnabled() const
{
    ...
    PageScaleConstraints constraints = frameHost()->pageScaleConstraintsSet().finalConstraints();
